export module gse.ide.command:command_registry;
...
auto gse::ide::command_registry::add(command_descriptor desc) -> void {
...
auto gse::ide::command_registry::invoke_by_name(std::string_view name) const -> bool {
...
auto gse::ide::command_registry::invoke_by_shortcut(std::string_view shortcut) const -> bool {
...
template <std::meta::info NamespaceInfo>
auto gse::ide::discover_commands(command_registry& reg) -> void {
	template for (constexpr auto m : std::define_static_array(std::meta::members_of(NamespaceInfo, std::meta::access_context::unchecked()))) {
		constexpr auto ann_name = gse::meta::find_class_template_annotation(m, ^^gse::ide::name);
		if constexpr (ann_name != std::meta::info{}) {
			...
			constexpr auto ann_shortcut = gse::meta::find_class_template_annotation(m, ^^gse::ide::shortcut);
			if constexpr (ann_shortcut != std::meta::info{}) {
				cmd.default_shortcut = [:ann_shortcut:]::value;
			}

			constexpr auto ann_description = gse::meta::find_class_template_annotation(m, ^^gse::ide::description);
			if constexpr (ann_description != std::meta::info{}) {
				cmd.description = [:ann_description:]::value;
			}

			constexpr auto ann_when = gse::meta::find_class_template_annotation(m, ^^gse::ide::when);
