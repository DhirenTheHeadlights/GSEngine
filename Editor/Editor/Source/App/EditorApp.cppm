...
export namespace gse::ide::workspace_system {
	...
	auto run(
		...
		gse::shared_view<build_runner::data> build_d
	) -> gse::async::task<>;
...
auto gse::ide::workspace_system::run(
	...
	const gse::shared_view<build_runner::data> build_d
) -> gse::async::task<> {
	if (!d.initialized) {
	...
	ctx.channels.push<gse::gui::menu_content>({
		...
		.layer = gse::render_layer::content,
		.build = [ws, graph = &d.graph, index = search_d.index, config, channels = ctx.channels, viewport_slot, game_running, building](gse::gui::builder& b) {
