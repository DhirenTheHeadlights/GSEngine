...
Status: settled 2026-07-20. **Phases 1, 2, 3a, 3b and 4 are done; phase 5 (packaging) is what remains** — its SDK spike passed, so nothing is gated on a research question any more. Goal: the editor is the installed all-in-one product — it ships the engine as a prebuilt SDK, creates/opens projects from a projects directory, and never writes runtime artifacts into its install dir or the repo.
...
| Game-code writes (locomotion_checkpoint.bin → CWD, LocomotionTrainer.cppm:27) | ~~CWD~~ **DONE** | `gse::config::project_data_path()` → `<project>/.gse/data` |
...
- **3b — relocate GoonSquad + Server. DONE 2026-07-31: [editor-first-restructure-phase3b.md](editor-first-restructure-phase3b.md).** ~~Purely the file move plus multi-target manifest support.~~ It was not purely a file move — locomotion was 61% of the game's source and lived as 12 partitions of the `gs` module, so extracting it was a module-identity change. It split cleanly by role (controller vs training, no cross-imports), which is what made the phase tractable. The humanoid cluster left for `%USERPROFILE%\GSEProjects\HumanoidLocomotion`; what stayed was renamed `GoonSquad` → **`Sandbox`** rather than deferring that rename to a later phase.
- **Engine binding is named, not just a path — added 2026-07-31 after a project was bricked by a deleted worktree.** `[engine] source = <path>` pointed at `.claude/worktrees/editor`; the worktree was removed and `BasePlanningTool` failed to configure with 450 lines of `CONFIGURE_DEPENDS` GLOB noise and three cascading CMake errors. Root cause: a raw path binding a project to a tree whose lifetime something else owns — which is the *normal* case here, since engine development lines are worktrees.
  - `%APPDATA%\GSE\engines.ini` (beside `recent_projects.ini`, same `layout_store` machinery, no new parser). The editor registers its own `root_dir()` on startup under its directory leaf, deduped by path, `-2`-suffixed on name collision. No setup step; running the editor from a new worktree registers it by existing.
  - Manifests carry **both** `name` and `source`. `name` wins when it resolves; `source` is the fallback. That keeps the registry's one-repair-for-N-projects property while leaving the file self-describing and degrading to the old behaviour if `engines.ini` is lost.
  - `load()` now takes the first candidate that **is** an engine tree (`Engine/cmake/GSEEngine.cmake` present) and records `engine_problem` otherwise, which `resolve()` logs at error level naming the offending `.gseproj`. A dead binding falls back to the editor's own engine, so the editor stays usable — previously `weakly_canonical` accepted a nonexistent path silently and everything downstream was quietly wrong.
  - `ensure_configured` compares the build tree's cached `GSE_ENGINE_DIR` against the project's current binding and reconfigures on mismatch. Without this the editor kept feeding ninja a poisoned cache; the 3.2 GB tree had to be deleted by hand.
  - The generated `CMakeLists.txt` guards the `include()` with an `EXISTS` check naming the `.gseproj`, replacing the three-error cascade.
  - Repair UI: the launcher opens straight into its Engine view when `engine_problem` is set, listing registered engines; picking one rewrites the manifest's `[engine]` section via `layout_store` (which preserves unowned sections) and relaunches.
...
- ~~Move Game/ out of the repo into the projects dir; locomotion tooling paths follow. Repo becomes Engine + Editor + Tools.~~ **Amended 2026-07-31, done the same day.** `Game/` stays in-repo as the engine's shipped physics-stress demo, driven by a free camera, and the multiplayer code stays with it until a multiplayer-first project claims it. Only the humanoid/locomotion cluster leaves. Repo becomes Engine + Editor + Tools **+ a demo** — and the demo is now `Sandbox/`, targets `Sandbox` / `SandboxServer`. See [phase 3b](editor-first-restructure-phase3b.md).
...
| ~~[BuildRunner.cppm](../Editor/Editor/Source/BuildRunner/BuildRunner.cppm) build cwd~~ | **DONE 2026-07-31** — `run_build_with_module_recovery` takes an explicit `source_dir`: `project_root()` for a game build, `gse::config::root_dir()` for the editor's self-rebuild |
| ~~[BuildRunner.cppm](../Editor/Editor/Source/BuildRunner/BuildRunner.cppm) launched-game cwd~~ | **DONE 2026-07-31** — `project_root()`. Was the editor's own tree, so every relative game write landed in the engine repo |
| [BuildRunner.cppm](../Editor/Editor/Source/BuildRunner/BuildRunner.cppm) relaunch cwd | editor's own tree — correct as-is |
| [Config.cpp:96](../Editor/Editor/Import/Config.cpp) Editor browse root | editor's own tree — correct as-is |

### Phase 4 — Product UX — **DONE 2026-07-31**
...
- ~~Hub/launcher on projectless launch~~ **DONE, and widened: it shows on _every_ launch**, not only a projectless one. The launcher is `project_screen(hub)`, which now also drives window size — it requests a content-sized window via `window_launcher_mode_request` and restores the previous geometry on dismiss. Three engine seams came out of it, all opt-in and inert for every other screen:
  - `screen::occludes()` — when the top screen occludes, `process_screen` skips `process_menu` entirely, so docked panels stop drawing. Needed because panels are pushed as `menu_content` and draw **outside** the screen stack; an opaque backdrop could not hide them, and shrinking the window would not have either.
  - `screen::wants_chrome()` (previously declared and never consumed) now carves `title_bar_height` off `body_rect`, and the engine draws the caption: fill, `title()`, min/max/close, and the `window_chrome_metrics_request`. `screen::draw_caption(builder&, area) -> float` hands the screen everything left of the buttons and takes back the width of any extra controls it drew. `editor_screen` adopted it, deleting its own bar, its three window buttons and `chrome_button`; its `build()` is now just the two keyboard shortcuts.
  - `screen::caption_exclusion_range(ctx, full_rect)` replaced an earlier plan to split `resize_exclude_*` into its own message. The editor derives that range as `screen_rect.top() - span`, which only worked because `body_rect` defaulted to the full viewport; once the caption is carved off, the origin silently shifts by `title_bar_height`. Handing the screen `full_rect` makes the correct origin unavoidable rather than something two sides must agree on.
  - **Close goes through `window_close_request` → `glfwSetWindowShouldClose`**, not `gse::shutdown()`. The caption lives in `gse.graphics:gui`, which sits below `gse.runtime`; the loop already turns `window_should_close()` into `gse::shutdown()`, so this is the same path reached through the layer that owns windows.
- ~~Open~~ **DONE.** `gse.win32::open_file_dialog` (needs an explicit `#include <commdlg.h>`; `WIN32_LEAN_AND_MEAN` excludes it) composed with C types only, per the no-STL-alongside-windows.h rule. `window_open_file_request` → handled in `window::tick`, which is already the main-thread function → `window_open_file_result` via `sched.make_channel_writer()`. Never called from the GUI system directly: a common dialog pumps its own message loop and would re-enter the custom NC-frame handler.
- ~~Templates (Blank first)~~ **DONE, two templates.** The second was not speculative — `BasePlanningTool` is a built 2D tool, and diffing its `Main.cpp` against the generated blank one gives exactly the Tool template: `.render_world = false`, `.simulate_world = false`, `import gse.system_manifest`, and a generated `Source/App.cppm` carrying the annotated-system idiom wired through `register_systems<^^ns::app>`. Module namespace is derived from the project name (lowercased, non-alphanumerics stripped, digit-prefixed if needed).
- ~~Engine symbol index layering~~ **DONE — see [Cross-project symbol reuse](#cross-project-symbol-reuse-2026-07-31).**

#### Cross-project symbol reuse (2026-07-31)

The goal was "an engine TU indexed once is reused by every project bound to that engine". Measured against the two live compilation databases (the editor's in-tree one, which **is** GoonSquad's, and `BasePlanningTool`'s), for the 366 engine TUs present in both:

| | Before | After |
|---|---|---|
| Cache keys matching across the two databases | **0 / 366** | **366 / 366** |
| Distinct keys (different TUs must not collide) | 366 | 366 |

It was never the semantic-vs-incidental argument classifier that was scoped. `make_check_command` **already** strips `-o`, `-c`, `-MD`, `-MF`, `-MT`, `-MQ` and `-fdeps-*`, every include path is absolute and points at the engine's shared `vcpkg_installed` (non-manifest mode, as intended), and the relative `-o`/`-fmodule-mapper` paths are textually equal because both trees place engine objects under `<build>/Engine/CMakeFiles/Engine.dir/`. Only two things broke reuse:

1. `check_command::fingerprint` folds in `directory` unconditionally.
2. The in-tree build passes `-std=c++26 -freflection -mavx2` **twice** and external projects once — see the flag-duplication note below.

Fix: `check_command` gains `semantic_fingerprint`, hashed from the kept tokens with operand-carrying flags folded into pairs, duplicates collapsed, and `directory` excluded. `tu_cache_path` and `tu_fingerprint` use it; `tu_cache_version` 9 → 10 invalidates the old entries.

**The non-obvious part, and the reason this needed measuring rather than reasoning.** The first implementation deduped while preserving first-occurrence order and scored **0/366**. The token *sets* were identical; the order was not, because the in-tree build's duplicate flags appear early and the project's appear late, so dedup leaves them in different positions. The shipped version splits the units: **include search paths keep their order** — `-I` order is resolution-sensitive, and collapsing it could serve symbols compiled against a different header — while all other flags are sorted and therefore order-independent. `tu_fingerprint` still folds in the plugin and every dependency's content fingerprint, so a stale hit cannot be served even if a key collided.

**Flag duplication is stale cache state, not a source bug.** `GSEEngine.cmake:36` supplies `-freflection -mavx2` through `add_compile_options`, which is what sharp edge #9 prescribes. The in-tree build dir *additionally* has `CMAKE_CXX_FLAGS:STRING=-std=c++26 -freflection -mavx2` baked into its cache, left from when those were preset-only via `CMAKE_CXX_FLAGS_INIT` — "consumed once, when `CMAKE_CXX_FLAGS` is first written to the cache". `BasePlanningTool`'s cache has it empty. Clearing it costs a full rebuild, since every object's command line changes; the normalization above makes the index immune either way.
...
### Phase 5 — Engine SDK + packaging (was phases 3 and 5; now distribution-only) — **spike passed, packaging outstanding**

Everything here serves *shipping to someone else*, not daily work. ~~The BMI-export risk is confined to this phase.~~ **The BMI-export risk is retired**: the spike proved relocated BMIs load and link. Detail, measured sizes and the remaining work list: [editor-first-restructure-phase5.md](editor-first-restructure-phase5.md).

- ~~**SDK spike (gate for this phase only)**~~ **PASSED 2026-07-31 via Path A — see [phase 5](editor-first-restructure-phase5.md).** A foreign TU compiled against 395 relocated BMIs, linked against relocated libs, and ran. The constraint it uncovered: GCC bakes each dependency BMI's *relative* path into the importing BMI and does not re-map transitive imports, so the SDK image must preserve the build tree's relative layout and the consumer must compile with its root at the SDK. The image is a relocatable copy of a build tree, not an arbitrary module directory. Path B (`install(... FILE_SET CXX_MODULES)` + find_package) is untried and inherits the same constraint.
...
8. ~~Game code needs a sanctioned project-data write API (checkpoint files etc.).~~ **CLOSED 2026-07-31.** `gse.manifest` gained a `project` key (written by `gse_write_manifest()`, defaulting to `CMAKE_SOURCE_DIR`; the engine repo's own build passes `Sandbox` because its source dir is the engine); `gse::config::project_root()` / `project_data_dir()` / `project_data_path(relative)` resolve from it, with `GSE_PROJECT_DIR` overriding. The paired half was that the editor launched games with cwd = **its own** tree, so relative game writes landed in the engine repo — the exact failure the restructure exists to prevent, and silent. Detail in [phase 3b](editor-first-restructure-phase3b.md#project-data-writes-2026-07-31).
