...
auto gs::spawn_domino_chain(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	for (int i = 0; i < 12; ++i) {
		...
		const auto tilt = (i == 0) ? gse::quat({ 0.f, 0.f, 1.f }, gse::radians(-0.8f)) : gse::quat();
		s.spawn(
			std::format("Domino {}", i + 1),
			gs::box(pos, gse::vec3<gse::length>(0.3f, 2.f, 1.f), gse::kilograms(30.f), tilt)
		);
	}
}
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			...
			s.spawn(
				...
				gs::box(pos, gse::vec3<gse::length>(gse::meters(1.f)), gse::kilograms(40.f))
			);
		}
	}
}
...
auto gs::spawn_slope_friction_test(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	s.spawn(
		...
		gs::box(ramp_position + resting_offset_for(ramp_tilt), box_size, gse::kilograms(50.f), ramp_tilt)
	);
	...
	s.spawn(
		...
		gs::box(steep_position + resting_offset_for(steep_tilt), box_size, gse::kilograms(50.f), steep_tilt)
	);
}
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			...
			s.spawn(
				...
				gs::box(pos, gse::vec3<gse::length>(gse::meters(1.f)), gse::kilograms(80.f))
			);
		}
	}
}
...
auto gs::spawn_spring_tests(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	for (int i = 0; i < 3; ++i) {
		...
		const auto bob_id = s.spawn(
			...
			gs::sphere(anchor_pos + gse::vec3<gse::length>(2.f, 0.f, 0.f), gse::meters(0.5f), gse::sphere_lod::lo)
		);
	...
	const auto chain_anchor =
		s.spawn("Spring Chain Anchor", gs::static_box(chain_anchor_pos, gse::vec3<gse::length>(0.5f, 0.5f, 0.5f)));
		...
		for (int ix = 0; ix < grid_x; ++ix) {
			for (int iz = 0; iz < grid_z; ++iz) {
				...
				s.spawn(
					...
					gs::box(pos, gse::vec3<gse::length>(gse::meters(1.f)), gse::kilograms(20.f))
				);
			}
		}
	}
...
auto gs::spawn_fixed_joint(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	const auto anchor_id =
		s.spawn("Fixed Anchor", gs::static_box(anchor_pos, gse::vec3<gse::length>(gse::meters(1.f))));
...
auto gs::spawn_distance_pendulum(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	const auto pivot_id =
		s.spawn("Distance Pivot", gs::static_box(pivot_pos, gse::vec3<gse::length>(0.5f, 0.5f, 0.5f)));
...
auto gs::spawn_pendulum_chain(gse::scene& s, const gse::vec3<gse::position>& origin) -> void {
	...
	const auto ceiling_id =
		s.spawn("Chain Ceiling", gs::static_box(ceiling_pos, gse::vec3<gse::length>(1.f, 0.5f, 1.f)));
	...
	for (int i = 0; i < chain_length; ++i) {
		...
		const auto link_id = s.spawn(
			...
			gs::box(link_pos, gse::vec3<gse::length>(0.6f, 0.6f, 0.6f), gse::kilograms(15.f))
		);
