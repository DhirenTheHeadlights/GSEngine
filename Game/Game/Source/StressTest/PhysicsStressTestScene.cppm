...
auto gs::physics_stress_test_scene::initialize() -> void {
	...
	build_sphere(this, "Bouncy Sphere",
...
auto gs::physics_stress_test_scene::build_inverted_mass_pyramid() const -> void {
	constexpr float x = -15.f;
	constexpr float z = 0.f;

	build_box(this, "Pyramid Light Base",
		gse::vec3<gse::position>(x, 0.5f, z),
		gse::vec3<gse::length>(gse::meters(1.f)),
		gse::kilograms(5.f));

	build_box(this, "Pyramid Mid",
		gse::vec3<gse::position>(x, 1.5f, z),
		gse::vec3<gse::length>(gse::meters(1.f)),
		gse::kilograms(50.f));

	build_box(this, "Pyramid Heavy Top",
		gse::vec3<gse::position>(x, 2.5f, z),
		gse::vec3<gse::length>(gse::meters(1.f)),
		gse::kilograms(500.f));
}

auto gs::physics_stress_test_scene::build_domino_chain() const -> void {
	...
	for (int i = 0; i < 12; ++i) {
		...
		build_box(this, std::format("Domino {}", i + 1),
...
auto gs::physics_stress_test_scene::build_funnel() const -> void {
	...
	build_static_box(this, "Funnel Left Wall",
		gse::vec3<gse::position>(cx - mid_offset, wall_height * 0.5f, cz),
		gse::vec3<gse::length>(wall_len, wall_height, 0.3f),
		left_rot);

	build_static_box(this, "Funnel Right Wall",
		gse::vec3<gse::position>(cx + mid_offset, wall_height * 0.5f, cz),
		gse::vec3<gse::length>(wall_len, wall_height, 0.3f),
		right_rot);

	build_static_box(this, "Funnel Back Wall",
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			...
			build_box(this, std::format("Funnel Box r{}c{}", row, col),
...
auto gs::physics_stress_test_scene::build_slope_friction_test() const -> void {
	...
	build_static_box(this, "Ramp 30deg", ramp_position, ramp_size, ramp_tilt);

	build_box(this, "Ramp Box Should Hold",
	...
	build_static_box(this, "Steep Ramp 45deg", steep_ramp_position, ramp_size, steep_tilt);

	build_box(this, "Steep Box Should Slide",
...
auto gs::physics_stress_test_scene::build_high_speed_impact_target() const -> void {
	constexpr float x = 0.f;
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			...
			build_box(this, std::format("Impact Wall r{}c{}", row, col),
...
auto gs::physics_stress_test_scene::build_spring_tests() const -> void {
	...
	for (int i = 0; i < 3; ++i) {
		...
		const auto anchor_id = build_static_box(this, std::format("Spring {} Anchor", labels[i]),
			gse::vec3<gse::position>(bx, 10.f, z),
			gse::vec3<gse::length>(0.5f, 0.5f, 0.5f)).identify();

		const auto bob_id = build_sphere(this, std::format("Spring {} Bob", labels[i]),
	...
	const auto chain_anchor = build_static_box(this, "Spring Chain Anchor",
	...
	for (int i = 0; i < 5; ++i) {
		...
		const auto link_id = build_sphere(this, std::format("Spring Chain Link {}", i),
...
auto gs::physics_stress_test_scene::build_tumbler() const -> void {
	...
	for (const auto& wall : walls) {
		build_box(this, std::format("Tumbler Wall {}", wall.suffix),
	...
	for (int ix = 0; ix < nx; ++ix) {
		for (int iy = 0; iy < ny; ++iy) {
			for (int iz = 0; iz < nz; ++iz) {
				...
				build_box(this, std::format("Tumbler Cube {}", content_id++),
...
auto gs::physics_stress_test_scene::build_box_grid() const -> void {
	...
	for (int layer = 0; layer < layers; ++layer) {
		for (int ix = 0; ix < grid_x; ++ix) {
			for (int iz = 0; iz < grid_z; ++iz) {
				...
				build_box(this, std::format("Grid L{}R{}C{}", layer, ix, iz),
