...
**Status.** `physics_stress` exists and is CPU-solver only, like every scenario (risk 3). Scene activation and the spawn helpers were already satisfied by Phase 1 — the bench activates `info.scene`, and `dev_spawn` already consumes both requests. What remains is the tuning state, synthetic input, and the render and locomotion scenarios.
...
Timing noise is the other output. p50 spans 8.67–9.14 ms across five identical runs, roughly a ±3% band on an otherwise idle machine; that is the floor a regression threshold has to clear, and it is measured rather than guessed. p99 is much looser (11.7–23.5 ms) — a single outlier frame moves it, so it is the wrong statistic to gate on.
