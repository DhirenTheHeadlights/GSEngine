...
- **Headless registered a different asset loader set than render.** Headless hardcoded `model`; render registered `graphics::asset_types + audio::asset_types`. Two hand-maintained lists of one fact, so headless silently lacked every type added since — `skinned_model` and `clip_asset` among them, which is why the first locomotion scenario asserted inside `spawn_character`. Giving headless the *whole* render pack was the wrong correction and was measured to be: it pulled texture and font loading into simulation runs, which put 162–401 ms `slow frame` spikes inside the measurement window and moved the world hash. `graphics::asset_types` is now `append<simulation_asset_types, presentation_asset_types>`, headless registers the simulation subset and render the whole thing, so there is still one declaration and the split is at the seam that actually exists.
