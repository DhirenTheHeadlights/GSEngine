...
3. **P3.1 — GPU jointed-pair filter** (3a): **filter DONE + verified correct & working (broad-phase, no regressions); humanoid-on-GPU BLOCKED by a separate deeper issue** — the GPU contact solver blows up the humanoid's foot↔ground contacts (proven by probes: ground-only contacts still explode; filter exonerated). It's the GPU↔CPU contact-fidelity/stability gap (the box stack was too forgiving to expose it). See Stage 3a status above. Next = characterize the GPU foot-ground blow-up on a minimal sensitive contact-parity scene.
...
5. **P3.3 — cheap per-env GPU reset** (3c).
