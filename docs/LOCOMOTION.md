...
**Implementation status (2026-06-14): built by a worker; reviewed — one critical correctness bug.** A worker implemented Phase 2 (grid spawn in `SandboxScene.cppm` via `world_training_setup` + `--locomotion-train` using `cfg.ppo.n_envs`; `trainer` extended to N envs with per-env `env_state` + per-env in-place reset). Review findings:

- 🔴 **GAE is computed across env boundaries (correctness bug — must fix before trusting any Phase-2 run).** The rollout buffer is one flat buffer filled by appending each env's transition in env-major order every tick, but `compute_gae` walks it as a single trajectory (`v_next = values[t+1]`), so adjacent slots are *different envs* and the GAE recursion chains one env's value onto another's. Advantages **and** returns are corrupted → wrong policy gradient and wrong critic targets. The single `last_bootstrap` (last env's value) is the same mistake. **Fix:** per-env rollout streams (each env's transitions contiguous) with GAE computed per env and a per-env bootstrap, then assembled into the PPO batch. *(Not fixed in this review pass — it's a real refactor, not a "simple" fix; flagged for a decision.)*
- 🟠 **Per-env reset restores only 10 of 17 bodies.** `take_env_snapshot` snapshots the bones in `skeleton_refs` (pelvis/torso/thighs/shins/feet/toes) — it omits the head + 6 arm bones, so after a reset the arms/head keep their fallen transforms and non-zeroed velocities, perturbing the otherwise-clean reset. `skeleton_refs` doesn't carry those ids — capture the full `skeleton_handle.bone_ids` (all 17) per env at spawn instead.
- 🟡 **Formatting (run clang-format):** the new code uses vertical alignment (`.clang-format` → `AlignConsecutiveAssignments: None`) and one-line `for`/getter bodies (`AllowShortBlocksOnASingleLine`/`AllowShortFunctionsOnASingleLine` = Never/None). No clang-format binary was available in the review environment — a format-on-save pass fixes all of it. **Fixed by hand:** `g_training_n_envs` is now `inline` (module-scope-variable rule — clang-format won't catch that). Minor: the grid spacing math uses raw `float` (`spacing = 4.0f`) before wrapping in `gse::meters` — unit-wrap it for purity.
