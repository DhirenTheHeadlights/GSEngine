...
- **2D/Texture.cppm:68** — `create_vulkan_resources` named a specific backend inside a backend-agnostic type that talks only to `gpu::device`/`gpu::image`/`gpu::sampler_desc`. **Fixed 2026-09-14**: renamed `create_device_resources`. *high*
