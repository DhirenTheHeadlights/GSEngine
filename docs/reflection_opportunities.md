...
### S1 Phase 2 — follow-through on the unit system

The initial pass consolidated `quantity_tag_traits<Tag>` into a `[[= quantity_spec_v<Dim, DefaultUnit, Parent, UnitFamily, Relative, Kind>]]` annotation. That replaced ~5 lines per quantity, but every quantity is still ~12 lines because the annotation itself is fat and the `_t` / default aliases are hand-written. Five follow-through cuts, ordered by blast radius:

#### 2a. `quantity_root` / `quantity_child` helpers (~120 lines)

Child tags currently restate `Dim`, `UnitFamily`, and `Relative` — all derivable from `Parent`. Split the spec into two compact forms:

```cpp
[[= quantity_root<^^dimi<0,0,0,1>, ^^radians, measurement>]]
struct angle_tag {};

[[= quantity_child<^^length_tag, relative>]]
struct displacement_tag {};

[[= quantity_child<^^displacement_tag, relative>]]
struct gap_tag {};
```

`quantity_child` resolves its `Dim`, `UnitFamily`, and `Relative` by reflecting the parent's own `quantity_spec`. `quantity_root` keeps the explicit dimension + default unit for the ~15 primitive tags. The 6-template-arg `quantity_spec_v` becomes an internal intermediate — users don't write it directly. ~20 child tags × 6 lines saved ≈ 120 lines.

#### 2b. `[[= default_unit]]` on the unit constant, drop forward decls (~35 lines)

Every tag has a forward declaration because the unit↔tag cycle can't close otherwise: units reference the tag, the tag annotation references a unit. Mark the canonical unit on the unit variable itself and the cycle breaks:

```cpp
[[= quantity_root<^^dimi<0,0,0,1>, measurement>]]
struct angle_tag {};
...
`quantity_traits<Tag>::default_unit` reflects the enclosing namespace for a `unit<Tag, ...>` constant carrying `[[= default_unit]]`. 35 forward decls deleted. The "modify one decl, forget the other" trap is structurally removed.

#### 2c. Auto-assembled `quantity_units<Tag>::units` tuple (capability, not lines)

Today only `time_tag` has the explicit `quantity_units<Tag>` specialization (`Units.cppm:278`); every other quantity silently falls through the `{:km/h}` branch in `Quantity.cppm:521`. A reflected scan of `^^gse` collects every `internal::unit<Tag, ...>` constant matching the tag and builds the tuple:

```cpp
template <typename Tag>
struct quantity_units {
    static constexpr auto units = collect_units_for<Tag, ^^gse>();
};
```

Unlocks `std::format("{:mph}", velocity)`, `{:N-m}` on torque, `{:lb-ft^2}` on inertia — every registered unit gets format-spec routing without per-tag glue. Feeds into UI dropdowns (Reshape 5) and TOML unit-suffix parsing.

#### 2d. Inject `_t` and default aliases via `consteval {}` (~70 lines)

Every quantity still writes its alias pair by hand:
...
Confirmed by grep: nobody uses the `_t<T, U>` template form — all 100% of call sites use the default-unit alias. So the alias pair can be generated by one block at module end:
...
Currently `force / mass` returns `generic_quantity<dimi<1,-2,0,0>>` and the named `acceleration` tag is laundered. Call sites must `quantity_cast<acceleration>(...)` to get it back — the CLAUDE.md "keep units flowing" rule enforced by hand rather than by the type system.

Build a `constexpr` dim→tag index from the same namespace scan that feeds 2c:
...
constexpr auto operator*(const Q1& lhs, const Q2& rhs) {
    ...
    constexpr auto tag_info = find_tag_by_dim<result_d>();
    if constexpr (!tag_info.is_null()) {
        using result_tag = [: tag_info :];
        using result_t = quantity_for_tag_t<result_tag, std::common_type_t<typename Q1::value_type, typename Q2::value_type>>;
...
The VBD solver's contact-lambda / angular-lambda chains stop losing tags mid-computation. `acceleration a = f / m;` becomes one line with no cast.

**Ambiguity note:** `torque` and `energy` share `dimi<2,-2,1,0>`. The index has to either (a) prefer the tag whose `unit_family` matches either input, or (b) fall back to `generic_quantity` when multiple tags match. Option (a) handles the common case; option (b) is the safe fallback.

### Phase 2 Rollout

1. **2a** first — pure template metaprogramming, no injection machinery needed. Immediate ~120-line win in `Units.cppm`.
2. **2b** next — closes the forward-decl cycle. Requires `quantity_traits<Tag>::default_unit` to learn namespace reflection.
3. **2c** — one-liner once 2b's namespace-scan primitive exists (`collect_units_for<Tag>()` is the same shape).
4. **2d** — try `queue_injection` for the alias template; fall back to `quantity_t<Tag>` helper if the fork rejects it. This is the riskiest slice; do it last when the rest have landed and call sites are confirmed stable.
5. **2e** — builds on the same namespace scan as 2c + 2d. Lands as a separate commit so tag-recovery behavior changes are bisectable.

Running total for Phase 2: ~225 lines cut from `Units.cppm` and two qualitative unlocks (universal format-spec routing, tag recovery without casts). The enabling primitive — a reflected namespace scan filtered by annotation — is the same machinery A3, A4, A5, B1, B4, C5, and most of Part II's reshapes all need.
