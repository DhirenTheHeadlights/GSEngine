...
template <typename T>
auto emit_enum_only(std::ostream& out) -> void {
...
template <typename T>
auto emit_struct_only(std::ostream& out) -> void {
...
auto main(const int argc, char** argv) -> int {
	...
	{
		std::ofstream out(out_dir / "common_types.slang");
		emit_pack(out, gse::shaders::common::shader_types{});
	}

	{
		std::ofstream out(out_dir / "forward_types.slang");
