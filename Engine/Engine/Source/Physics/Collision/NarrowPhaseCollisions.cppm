...
export namespace gse::narrow_phase_collision {
	...
	struct shape_data {
		...
		const physics::convex_hull* hull = nullptr;
...
namespace gse::narrow_phase_collision {
	...
	struct minkowski_point {
		vec3<length> point;
		vec3<position> support_a;
		vec3<position> support_b;
	};

	struct face_info {
		std::uint8_t face_index = 0;
		std::array<vec3<position>, 4> vertices;
		vec3f normal;
	...
	auto hull_sat_to_result(
		const physics::hull_sat& sat
	) -> std::optional<sat_result>;

	auto box_hull_speculative(
		const shape_data& box,
		const physics::box_shape& box_shape,
		const shape_data& hull,
		length margin
	) -> std::optional<sat_result>;

	auto sphere_hull_speculative(
	...
	auto capsule_hull_speculative(
		const bounding_box& cap_bb,
		length half_height,
		length radius,
		const shape_data& hull,
		length margin
	...
	auto hull_hull_speculative(
		const shape_data& a,
		const shape_data& b,
		length margin
	) -> std::optional<sat_result>;

	auto box_hull_manifold(
		const shape_data& box,
		const physics::box_shape& box_shape,
		const shape_data& hull,
		const vec3f& normal
	...
	auto hull_hull_manifold(
		const shape_data& a,
		const shape_data& b,
		const vec3f& normal
	) -> contact_manifold;

	constexpr std::uint8_t hull_surface_index = 9;
	...
	auto capsule_capsule_speculative(
		...
		length ha,
		length ra,
		const bounding_box& bb_b,
		length hb,
		length rb,
		length margin
	) -> std::optional<sat_result>;

	auto box_capsule_speculative(
		const bounding_box& bb,
		const bounding_box& cap_bb,
		length half_h,
		length radius,
	...
	auto sphere_capsule_speculative(
		const vec3<position>& sph_center,
		length sph_r,
		const bounding_box& cap_bb,
		length cap_h,
		length cap_r,
...
auto gse::narrow_phase_collision::support_obb(const bounding_box& bb, const vec3f& dir) -> vec3<position> {
	auto result = bb.center();
...
auto gse::narrow_phase_collision::clip_polygon(const std::inplace_vector<clip_vertex, 9>& subject, const plane& p, const bool keep_greater, const bool tag_reference_side, const std::uint8_t reference_side) -> std::inplace_vector<clip_vertex, 9> {
	...
	length prev_dist = dot(p.normal, prev.point - p.point);
	bool prev_inside = inside(prev_dist);

	for (auto curr : subject) {
		const length curr_dist = dot(p.normal, curr.point - p.point);
...
auto gse::narrow_phase_collision::build_clipped_face_contacts(const bounding_box& bb1, const bounding_box& bb2, const vec3f& collision_normal, const bool prefer_reference_a) -> std::optional<clipped_face_contacts> {
	...
	const bool reference_is_a = std::abs(info1.max_dot - info2.max_dot) <= reference_face_preference_margin ? prefer_reference_a : info1.max_dot > info2.max_dot;
	...
	auto reference_center = reference_info.vertices[0];
	for (std::size_t i = 1; i < reference_info.vertices.size(); ++i) {
		reference_center += (reference_info.vertices[i] - reference_info.vertices[0]) * 0.25f;
	}
	...
	if (constexpr float slanted_face_alignment_threshold = 0.985f; !polygon.empty() && face_alignment < slanted_face_alignment_threshold) {
		...
		for (std::size_t i = 0; i < incident_info.vertices.size(); ++i) {
			const length edge_length =
				magnitude(incident_info.vertices[(i + 1) % incident_info.vertices.size()] - incident_info.vertices[i]);
		...
		for (const auto& vertex : polygon) {
			const auto vertex_distance = dot(reference_normal, vertex.point - reference_plane.point);
...
auto gse::narrow_phase_collision::sat_penetration(const bounding_box& bb1, const bounding_box& bb2) -> std::pair<vec3f, length> {
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			test_axis(
				cross(bb1.obb().axes[i], bb2.obb().axes[j]),
...
auto gse::narrow_phase_collision::sat_speculative(const bounding_box& bb1, const bounding_box& bb2, const length speculative_margin) -> std::optional<sat_result> {
	...
	auto half2 = bb2.half_extents();
	for (int i = 0; i < 3; ++i) {
		if (!test_axis(bb1.obb().axes[i], sat_axis_source::face, half[i])) {
			return std::nullopt;
		}
		if (!test_axis(bb2.obb().axes[i], sat_axis_source::face, half2[i])) {
			return std::nullopt;
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			if (!test_axis(cross(bb1.obb().axes[i], bb2.obb().axes[j]), sat_axis_source::cross, length{})) {
				return std::nullopt;
			}
		}
	}
...
auto gse::narrow_phase_collision::generate_manifold(const bounding_box& bb1, const bounding_box& bb2, const vec3f& normal, const separation separation, const bool prefer_reference_a) -> contact_manifold {
	...
	const auto quantize = [](const length value) -> int {
		const float v = internal::to_storage(value);
	...
	if (const auto clipped = build_clipped_face_contacts(bb1, bb2, normal, prefer_reference_a)) {
		candidates.reserve(clipped->vertices.size());
		for (const auto& vertex : clipped->vertices) {
			const vec3<position> position_on_a = clipped->reference_is_a ? vertex.point : (vertex.point - normal * contact_gap);
			const vec3<position> position_on_b = clipped->reference_is_a ? (vertex.point + normal * contact_gap) : vertex.point;
...
auto gse::narrow_phase_collision::capsule_endpoints(const bounding_box& bb, const length half_height) -> std::pair<vec3<position>, vec3<position>> {
...
auto gse::narrow_phase_collision::closest_point_on_segment(const vec3<position>& a, const vec3<position>& b, const vec3<position>& p) -> vec3<position> {
	const auto ab = b - a;
...
auto gse::narrow_phase_collision::query_obb(const bounding_box& bb, const vec3<position>& point) -> obb_query_result {
	const auto o = bb.obb();
	const auto he = bb.half_extents();
	const auto diff = point - o.center;

	std::array<displacement, 3> local;
	bool inside = true;
	for (int i = 0; i < 3; ++i) {
	...
	int min_axis = 0;
	auto min_depth = he[0] - abs(local[0]);
	for (int i = 1; i < 3; ++i) {
		if (auto depth = he[i] - abs(local[i]); depth < min_depth) {
			min_depth = depth;
			min_axis = i;
		}
	}

	const float sign = local[min_axis] >= displacement{} ? 1.f : -1.f;
	auto cp = o.center;
	for (int i = 0; i < 3; ++i) {
		const auto offset = (i == min_axis) ? he[i] * sign : local[i];
		cp += o.axes[i] * offset;
	}
	return { cp, o.axes[min_axis] * sign, -min_depth };
}

auto gse::narrow_phase_collision::segment_obb_query(const bounding_box& bb, const vec3<position>& seg_start, const vec3<position>& seg_end) -> std::pair<vec3<position>, obb_query_result> {
...
auto gse::narrow_phase_collision::classify_capsule_feature(const float t) -> std::pair<feature_type, std::uint8_t> {
	...
	if (t < cap_threshold) {
		return { feature_type::vertex, capsule_cap_a_index };
	}
	if (t > 1.f - cap_threshold) {
		return { feature_type::vertex, capsule_cap_b_index };
	}
	return { feature_type::edge, capsule_barrel_index };
}

auto gse::narrow_phase_collision::sphere_sphere_speculative(const vec3<position>& ca, const length ra, const vec3<position>& cb, const length rb, const length margin) -> std::optional<sat_result> {
	const auto diff = cb - ca;
	const auto dist = magnitude(diff);
	const separation separation = ra + rb - dist;
...
auto gse::narrow_phase_collision::capsule_capsule_speculative(const bounding_box& bb_a, const length ha, const length ra, const bounding_box& bb_b, const length hb, const length rb, const length margin) -> std::optional<sat_result> {
...
auto gse::narrow_phase_collision::sphere_capsule_speculative(const vec3<position>& sph_center, const length sph_r, const bounding_box& cap_bb, const length cap_h, const length cap_r, const length margin) -> std::optional<sat_result> {
	...
	const auto dist = magnitude(diff);
	const separation separation = sph_r + cap_r - dist;
...
auto gse::narrow_phase_collision::sphere_sphere_manifold(const vec3<position>& ca, const length ra, const vec3<position>& cb, const length rb, const vec3f& normal, const separation separation) -> contact_manifold {
	contact_manifold manifold;
...
auto gse::narrow_phase_collision::box_sphere_manifold(const bounding_box& bb, const vec3<position>& center, const length radius, const vec3f& normal, const separation separation) -> contact_manifold {
	contact_manifold manifold;
	auto [tu, tv] = compute_tangent_basis(normal);
	manifold.tangent_u = tu;
	manifold.tangent_v = tv;
...
auto gse::narrow_phase_collision::capsule_capsule_manifold(const bounding_box& bb_a, const length ha, const length ra, const bounding_box& bb_b, const length hb, const length rb, const vec3f& normal, const separation separation) -> contact_manifold {
	contact_manifold manifold;
	auto [tu, tv] = compute_tangent_basis(normal);
	manifold.tangent_u = tu;
	manifold.tangent_v = tv;
...
auto gse::narrow_phase_collision::box_capsule_manifold(const bounding_box& bb, const bounding_box& cap_bb, const length cap_h, const length cap_r, const vec3f& normal, const separation separation) -> contact_manifold {
	contact_manifold manifold;
	auto [tu, tv] = compute_tangent_basis(normal);
	manifold.tangent_u = tu;
	manifold.tangent_v = tv;
...
auto gse::narrow_phase_collision::sphere_capsule_manifold(const vec3<position>& sph_center, const length sph_r, const bounding_box& cap_bb, const length cap_h, const length cap_r, const vec3f& normal, const separation separation) -> contact_manifold {
	contact_manifold manifold;
	auto [tu, tv] = compute_tangent_basis(normal);
	manifold.tangent_u = tu;
	manifold.tangent_v = tv;
...
auto gse::narrow_phase_collision::pose_of(const shape_data& s) -> physics::hull_pose {
	return {
		.center = s.tc->position,
		.orientation = s.tc->orientation,
		.body_index = s.body_index
	};
}

auto gse::narrow_phase_collision::hull_sat_to_result(const physics::hull_sat& sat) -> std::optional<sat_result> {
	if (!sat.hit) {
		return std::nullopt;
	}
	return sat_result{
		...
		.is_speculative = sat.is_speculative
	};
}

auto gse::narrow_phase_collision::box_hull_speculative(const shape_data& box, const physics::box_shape& box_shape, const shape_data& hull, const length margin) -> std::optional<sat_result> {
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return std::nullopt;
	}
	const auto box_poly = physics::make_box_polytope(box_shape.size * 0.5f);
	return hull_sat_to_result(
		physics::polytope_sat(box_poly.view(), pose_of(box), physics::hull_polytope(*hull.hull), pose_of(hull), margin)
	);
}

auto gse::narrow_phase_collision::sphere_hull_speculative(const vec3<position>& center, const length radius, const shape_data& hull, const length margin) -> std::optional<sat_result> {
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return std::nullopt;
	}
	const auto query = physics::query_polytope(physics::hull_polytope(*hull.hull), pose_of(hull), center);
	const separation separation = radius - query.signed_distance;
	if (separation < -margin) {
		return std::nullopt;
	}
	return sat_result{
...
auto gse::narrow_phase_collision::capsule_hull_speculative(const bounding_box& cap_bb, const length half_height, const length radius, const shape_data& hull, const length margin) -> std::optional<sat_result> {
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return std::nullopt;
	}
	const auto [seg_start, seg_end] = capsule_endpoints(cap_bb, half_height);
	vec3<position> best_point;
	const auto query = physics::segment_polytope_query(
		physics::hull_polytope(*hull.hull),
		pose_of(hull),
		seg_start,
		seg_end,
		best_point
...
auto gse::narrow_phase_collision::hull_hull_speculative(const shape_data& a, const shape_data& b, const length margin) -> std::optional<sat_result> {
	if (a.hull == nullptr || b.hull == nullptr || !a.hull->valid() || !b.hull->valid()) {
		return std::nullopt;
	}
	return hull_sat_to_result(
		physics::polytope_sat(
			physics::hull_polytope(*a.hull),
			pose_of(a),
			physics::hull_polytope(*b.hull),
			pose_of(b),
			margin
		)
	);
...
auto gse::narrow_phase_collision::box_hull_manifold(const shape_data& box, const physics::box_shape& box_shape, const shape_data& hull, const vec3f& normal) -> contact_manifold {
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return {};
	}
	const auto box_poly = physics::make_box_polytope(box_shape.size * 0.5f);
	return physics::polytope_manifold(
		box_poly.view(),
		pose_of(box),
		physics::hull_polytope(*hull.hull),
		pose_of(hull),
...
auto gse::narrow_phase_collision::sphere_hull_manifold(const vec3<position>& center, const length radius, const shape_data& hull, const vec3f& normal) -> contact_manifold {
	contact_manifold manifold;
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return manifold;
	}
	...
	manifold.add_point(
		contact_point{
			.position_on_a = center + normal * radius,
			.position_on_b = query.closest,
			.normal = normal,
			.separation = meters(0.f),
			.feature =
				feature_id{
...
auto gse::narrow_phase_collision::capsule_hull_manifold(const bounding_box& cap_bb, const length half_height, const length radius, const shape_data& hull, const vec3f& normal) -> contact_manifold {
	contact_manifold manifold;
	if (hull.hull == nullptr || !hull.hull->valid()) {
		return manifold;
	}
	...
	float t_param = 0.5f;
	if (const auto seg_diff = seg_end - seg_start; magnitude(seg_diff) > meters(1e-6f)) {
		t_param = std::clamp<float>(dot(best_point - seg_start, seg_diff) / dot(seg_diff, seg_diff), 0.f, 1.f);
	}
	...
	manifold.add_point(
		contact_point{
			.position_on_a = best_point + normal * radius,
			.position_on_b = query.closest,
			.normal = normal,
			.separation = meters(0.f),
			.feature =
...
auto gse::narrow_phase_collision::hull_hull_manifold(const shape_data& a, const shape_data& b, const vec3f& normal) -> contact_manifold {
	if (a.hull == nullptr || b.hull == nullptr || !a.hull->valid() || !b.hull->valid()) {
		return {};
	}
	return physics::polytope_manifold(
...
auto gse::narrow_phase_collision::speculative_test(const shape_data& a, const shape_data& b, const length margin) -> std::optional<sat_result> {
	...
	gse::match(*lo.shape)
		.if_is([&](const physics::box_shape& lo_box) {
			...
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::hull_shape&) {
					result = box_hull_speculative(lo, lo_box, hi, margin);
				});
		})
		.else_if_is([&](const physics::sphere_shape& lo_sph) {
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::capsule_shape& hi_cap) {
					const bounding_box hi_bb(*hi.tc);
					result = sphere_capsule_speculative(
						lo.tc->position,
						lo_sph.radius,
						hi_bb,
						hi_cap.half_height,
				...
				.else_if_is([&](const physics::hull_shape&) {
					result = sphere_hull_speculative(lo.tc->position, lo_sph.radius, hi, margin);
				});
		})
		.else_if_is([&](const physics::capsule_shape& lo_cap) {
			...
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::hull_shape&) {
					result = capsule_hull_speculative(lo_bb, lo_cap.half_height, lo_cap.radius, hi, margin);
				});
		})
		.else_if_is([&](const physics::hull_shape&) {
...
auto gse::narrow_phase_collision::generate_shape_manifold(const shape_data& a, const shape_data& b, const vec3f& normal, const separation separation) -> contact_manifold {
	const bool swap = a.shape->index() > b.shape->index();
	const auto& lo = swap ? b : a;
	const auto& hi = swap ? a : b;
	const auto n = swap ? -normal : normal;
	const bool prefer_reference_lo = lo.body_index < hi.body_index;
	...
	gse::match(*lo.shape)
		.if_is([&](const physics::box_shape& lo_box) {
			...
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::hull_shape&) {
					manifold = box_hull_manifold(lo, lo_box, hi, n);
				});
		})
		.else_if_is([&](const physics::sphere_shape& lo_sph) {
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::hull_shape&) {
					manifold = sphere_hull_manifold(lo.tc->position, lo_sph.radius, hi, n);
				});
		})
		.else_if_is([&](const physics::capsule_shape& lo_cap) {
			const bounding_box lo_bb(*lo.tc);
			gse::match(*hi.shape)
				...
				.else_if_is([&](const physics::hull_shape&) {
					manifold = capsule_hull_manifold(lo_bb, lo_cap.half_height, lo_cap.radius, hi, n);
				});
		})
		.else_if_is([&](const physics::hull_shape&) {
			gse::match(*hi.shape).if_is([&](const physics::hull_shape&) {
