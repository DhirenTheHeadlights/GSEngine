...
auto gse::physics::system::initialize(const initialize_phase& phase, state& s) -> void {
	...
	phase.channels.push(save::register_property{
		.category = "Physics",
		.name = "Use VBD Solver",
		.description = "Use the VBD hybrid solver instead of sequential impulses",
		.ref = &s.use_vbd_solver,
		.type = typeid(bool)
	});

	if (const auto* save_state = phase.try_state_of<save::state>()) {
		s.update_phys = save_state->read("Physics", "Update Physics", true);
		s.use_vbd_solver = save_state->read("Physics", "Use VBD Solver", false);
	}
...
auto gse::physics::system::update(update_phase& phase, state& s) -> void {
	...
	if (s.use_vbd_solver) {
		...
		phase.schedule([steps](chunk<motion_component> motion, chunk<collision_component> collision) {
			update_legacy(steps, motion, collision);
		});
	}
}

auto gse::physics::update_legacy(
	const int steps,
	chunk<motion_component>& motion,
	chunk<collision_component>& collision
) -> void {
