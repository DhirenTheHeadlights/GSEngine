...
auto gse::trace::begin_async(const id& id, std::uint64_t key) -> void {
	ensure_tls_registered();
...
auto gse::trace::counter(const id& id, double value) -> void {
	ensure_tls_registered();
...

	{
		...
		for (auto* tb : tls_registry) {
			...
			if (!tb->events.empty()) {
				fs.merged.insert(
					fs.merged.end(),
					std::make_move_iterator(tb->events.begin()),
