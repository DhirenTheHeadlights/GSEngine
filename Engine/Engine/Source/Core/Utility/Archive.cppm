...
export namespace gse {
    ...
    struct binary_writer {
        ...
        explicit binary_writer(std::ofstream& stream) : m_stream(stream) {}

        binary_writer(std::ofstream& stream, std::uint32_t magic, std::uint32_t version) : m_stream(stream) {
            *this & magic & version;
        }
        ...
        auto operator&(const std::string& str) -> binary_writer& {
            ...
            m_stream.write(str.c_str(), static_cast<std::streamsize>(size));
            return *this;
        }
    ...
    struct binary_reader {
        ...
        explicit binary_reader(std::ifstream& stream) : m_stream(stream) {}

        binary_reader(std::ifstream& stream, std::uint32_t expected_magic, std::uint32_t expected_version,
                      std::string_view path, std::source_location loc = std::source_location::current())
            : m_stream(stream) {
        ...
        auto operator&(std::string& str) -> binary_reader& {
            ...
            str.resize(size);
            if (size > 0) m_stream.read(str.data(), size);
            return *this;
        }
