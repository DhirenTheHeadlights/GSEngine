...
export namespace gse::animation {
	auto create_graph(
		const animation_graph& graph
	) -> id;

	auto build(
		const graph_builder& builder
	) -> id;

	auto set_parameter(
		controller_component& ctrl,
...
auto gse::animation::build(const graph_builder& builder) -> id {
	...
	for (const auto& [name, clip, speed, loop] : collected_states | std::views::values) {
	...
	for (const auto& t : builder.transitions()) {
		if (t.from_any) {
			for (const auto& name : collected_states | std::views::keys) {
...
auto gse::animation::set_parameter(controller_component& ctrl, const std::string_view name, const bool value) -> void {
	ctrl.parameters[std::string(name)] = {
		.value = value,
		.is_trigger = false
	};
}

auto gse::animation::set_parameter(controller_component& ctrl, const std::string_view name, const float value) -> void {
	ctrl.parameters[std::string(name)] = {
		.value = value,
		.is_trigger = false
	};
...
auto gse::animation::set_trigger(controller_component& ctrl, const std::string_view name) -> void {
	ctrl.parameters[std::string(name)] = {
		.value = true,
		.is_trigger = true
	};
