...
export namespace gse::gpu {
	struct context {
		struct[[= gse::settings::category<"Graphics">{}]] data {
			...
			[[= gse::settings::describe<"Vulkan device tracking and naming options.">{}]] vulkan::device::settings
				device_settings;
		...
		static auto run(run_context& ctx, const window::data& window_s, data& d) -> async::task<>;

		static auto shutdown(shutdown_context& phase, data& d) -> void;

		[[nodiscard]] static auto begin_frame(data& d, window::data& window_s)
		...
		static auto end_frame(data& d, window::data& window_s) -> void;

		static auto on_swap_chain_recreate(const data& d, swap_chain_recreate_callback callback) -> void;

		static auto wait_idle(const data& d) -> void;

		[[nodiscard]] static auto device_handle(const gpu::device& device) -> handle<vulkan::device>;
	};
	...
	[[nodiscard]] auto on_gpu(channel_writer& channels) -> on_gpu_awaitable;
}
