...
auto gse::verify_uniform_block(const shader& s, const std::string_view block_name) -> bool {
	...
	template for (constexpr auto m : std::meta::nonstatic_data_members_of(^^T, std::meta::access_context::unchecked())) {
		const std::string member_name(std::meta::identifier_of(m));
		constexpr auto cpp_offset = static_cast<std::uint32_t>(std::meta::offset_of(m).bytes);
		constexpr auto cpp_size = static_cast<std::uint32_t>(sizeof([:std::meta::type_of(m):]));

		const auto it = block.members.find(member_name);
		if (it == block.members.end()) {
			log::println(
				log::level::error,
				log::category::assets,
				"Layout mismatch in '{}': member '{}' missing from shader block",
				block_name,
				member_name
			);
			ok = false;
		...
		const auto& shader_member = it->second;
		if (shader_member.offset != cpp_offset) {
			log::println(
				log::level::error,
				log::category::assets,
				"Layout mismatch in '{}.{}': C++ offset {}, shader offset {}",
				block_name,
				member_name,
				cpp_offset,
				shader_member.offset
			);
			ok = false;
		}
		if (shader_member.size != cpp_size) {
			log::println(
				log::level::error,
				log::category::assets,
				"Layout mismatch in '{}.{}': C++ size {}, shader size {}",
				block_name,
				member_name,
				cpp_size,
				shader_member.size
	...
	if (sizeof(T) != block.size) {
		log::println(
			log::level::warning,
			log::category::assets,
			"Total size mismatch for block '{}': C++ sizeof {}, shader block size {}",
			block_name,
			sizeof(T),
			block.size
		);
