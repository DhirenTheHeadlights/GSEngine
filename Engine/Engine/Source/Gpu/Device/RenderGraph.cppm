...
export namespace gse::vulkan {
	...
	public:
		...
		auto commit(
			const descriptor_writer& writer,
			const pipeline_handle& p,
			std::uint32_t set_index = 0
		) const -> void;
...
auto gse::vulkan::recording_context::copy_buffer(const buffer_resource& src, const buffer_resource& dst, const std::size_t size, const std::size_t src_offset, const std::size_t dst_offset) const -> void {
	m_cmd.copyBuffer(src.buffer, dst.buffer, vk::BufferCopy{
...
auto gse::vulkan::recording_context::capture_swapchain(
	...
	const auto dst_buffer = dst.buffer;
...
auto gse::vulkan::recording_context::blit_swapchain_to_image(const gpu::swap_chain& swapchain, const gpu::frame& frame, const image_resource& dst, const vec2u dst_extent) const -> void {
	...
	const vk::ImageBlit region{
		...
		.dstOffsets = std::array{
			vk::Offset3D{0, 0, 0},
			vk::Offset3D{static_cast<int>(dst_extent.x()), static_cast<int>(dst_extent.y()), 1}
		}
	};
	m_cmd.blitImage(
...
auto gse::vulkan::recording_context::bind_index_buffer(const vk::Buffer buffer, const vk::DeviceSize offset, const vk::IndexType index_type) const -> void {
	m_cmd.bindIndexBuffer2KHR(buffer, offset, vk::WholeSize, index_type);
}
...
auto gse::vulkan::recording_context::bind_pipeline(const vk::PipelineBindPoint point, const pipeline_handle& p) const -> void {
	m_cmd.bindPipeline(point, p.pipeline);
}

auto gse::vulkan::recording_context::bind_descriptors(const vk::PipelineBindPoint point, const pipeline_handle& p, const descriptor_region& region, const std::uint32_t set_index) const -> void {
	assert(region, std::source_location::current(), "Cannot bind null descriptor region");
	region.heap->bind(m_cmd, point, p.layout, set_index, region);
}

auto gse::vulkan::recording_context::push(const pipeline_handle& p, const gpu::cached_push_constants& cache) const -> void {
...
auto gse::vulkan::recording_context::draw_indirect(const buffer_resource& buf, const std::size_t offset, const std::uint32_t draw_count, const std::uint32_t stride) const -> void {
	m_cmd.drawIndexedIndirect(buf.buffer, offset, draw_count, stride);
}

auto gse::vulkan::recording_context::bind(const pipeline_handle& p) const -> void {
...
auto gse::vulkan::recording_context::bind_descriptors(const pipeline_handle& p, const descriptor_region& region, const std::uint32_t set_index) const -> void {
	assert(region, std::source_location::current(), "Cannot bind null descriptor region");
	const auto point = p.point == gpu::bind_point::graphics
		? vk::PipelineBindPoint::eGraphics
		: vk::PipelineBindPoint::eCompute;
...
auto gse::vulkan::recording_context::bind_vertex(const buffer_resource& buf, const std::size_t offset) const -> void {
	const vk::Buffer buffers[]{ buf.buffer };
...
auto gse::vulkan::recording_context::bind_index(const buffer_resource& buf, const gpu::index_type type, const std::size_t offset) const -> void {
	...
	m_cmd.bindIndexBuffer2KHR(buf.buffer, offset, vk::WholeSize, vk_type);
}
...
auto gse::vulkan::recording_context::begin_rendering(const vk::ImageView depth_view, const vec2u extent, const float clear_depth_value) const -> void {
...
auto gse::vulkan::recording_context::commit(const descriptor_writer& writer, const pipeline_handle& p, const std::uint32_t set_index) const -> void {
	const auto point = p.point == gpu::bind_point::graphics
		? vk::PipelineBindPoint::eGraphics
		: vk::PipelineBindPoint::eCompute;
	writer.commit(m_cmd, point, p.layout, set_index);
}

auto gse::vulkan::recording_context::bind_vertex_buffer(const buffer_resource& buf, const vk::DeviceSize offset) const -> void {
	const vk::Buffer buffers[]{ buf.buffer };
	const vk::DeviceSize offsets[]{ offset };
	m_cmd.bindVertexBuffers(0, 1, buffers, offsets);
}

auto gse::vulkan::recording_context::bind_index_buffer(const buffer_resource& buf, const vk::DeviceSize offset, const vk::IndexType index_type) const -> void {
	m_cmd.bindIndexBuffer2KHR(buf.buffer, offset, vk::WholeSize, index_type);
}
...
auto gse::gpu::storage_read(const vulkan::buffer_resource& buf, const pipeline_stage stage) -> vulkan::resource_usage {
	return vulkan::storage_read(buf, vulkan::to_vk_stage(stage));
}

auto gse::gpu::storage_write(const vulkan::buffer_resource& buf, const pipeline_stage stage) -> vulkan::resource_usage {
	return vulkan::storage(buf, vulkan::to_vk_stage(stage));
}

auto gse::gpu::transfer_read(const vulkan::buffer_resource& buf) -> vulkan::resource_usage {
	return vulkan::transfer_read(buf);
}

auto gse::gpu::transfer_write(const vulkan::buffer_resource& buf) -> vulkan::resource_usage {
	return vulkan::transfer_write(buf);
}

auto gse::gpu::sampled(const vulkan::image_resource& img, const pipeline_stage stage) -> vulkan::resource_usage {
	return vulkan::sampled(img, vulkan::to_vk_stage(stage));
}

auto gse::gpu::indirect_read(const vulkan::buffer_resource& buf, const pipeline_stage stage) -> vulkan::resource_usage {
	return vulkan::indirect_read(buf, vulkan::to_vk_stage(stage));
}
		...
		for (std::size_t si = 0; si < sorted.size(); ++si) {
			...
			if (is_graphics_pass) {
				...
				if (pass.depth_output) {
					...
					depth_att = vk::RenderingAttachmentInfo{
						.imageView = m_swapchain->depth_image().view,
						.imageLayout = vk::ImageLayout::eGeneral,
						.loadOp = vk_load,
						.storeOp = vk::AttachmentStoreOp::eStore,
						.clearValue = clear_val
