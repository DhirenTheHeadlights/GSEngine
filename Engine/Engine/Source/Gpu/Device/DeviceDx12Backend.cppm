...
auto gse::gpu::dx12_device_backend::create_shared_surface(const gpu::shared_surface_desc& desc) const -> std::expected<gpu::shared_surface, std::string> {
	return device->create_shared_surface(desc);
}

auto gse::gpu::dx12_device_backend::import_shared_surface(const gpu::shared_surface_desc& desc, void* handle) const -> std::expected<gpu::shared_surface, std::string> {
	return device->import_shared_surface(desc, handle);
}

auto gse::gpu::dx12_device_backend::destroy_shared_surface(const gpu::shared_surface& surface) const -> void {
...
auto gse::gpu::dx12_device_backend::export_semaphore_handle(const gpu::handle<gpu::semaphore> semaphore) const -> std::expected<void*, std::string> {
	return device->export_semaphore_handle(semaphore);
}

auto gse::gpu::dx12_device_backend::import_semaphore_handle(void* handle) -> std::expected<gpu::handle<gpu::semaphore>, std::string> {
...
auto gse::gpu::dx12_device_backend::create_swapchain(gpu::surface, const vec2i framebuffer_size, const gpu::present_mode mode, const gpu::swap_chain_handle old_handle) -> gpu::expected<gpu::swap_chain_info> {
	return swapchain.create(framebuffer_size, mode, old_handle);
}

auto gse::gpu::dx12_device_backend::boot_surface() const -> gpu::surface {
	return {};
}

auto gse::gpu::dx12_device_backend::recreate_surface(const window::window_surface&, gpu::swap_chain_handle) -> gpu::surface {
	return {};
}

auto gse::gpu::dx12_device_backend::create_surface(native_window_handle) -> gpu::surface {
	return {};
}

auto gse::gpu::dx12_device_backend::destroy_surface(gpu::surface) -> void {
}
...
auto gse::gpu::dx12_device_backend::buffer_slot(const gpu::handle<gpu::buffer> buffer) const -> gpu::bindless_slot {
	return device->buffer_slot(buffer);
}

auto gse::gpu::dx12_device_backend::buffer_address(const gpu::handle<gpu::buffer> buffer) const -> gpu::device_address {
	return device->buffer_address(buffer);
}

auto gse::gpu::dx12_device_backend::buffer_size(const gpu::handle<gpu::buffer> buffer) const -> gpu::device_size {
	return device->buffer_size(buffer);
}

auto gse::gpu::dx12_device_backend::buffer_mapped(const gpu::handle<gpu::buffer> buffer) const -> std::byte* {
	return device->buffer_mapped(buffer);
}

auto gse::gpu::dx12_device_backend::image_sampled_slot(const gpu::handle<gpu::image> image) const -> gpu::bindless_slot {
	return device->image_sampled_slot(image);
}

auto gse::gpu::dx12_device_backend::image_storage_slot(const gpu::handle<gpu::image> image) const -> gpu::bindless_slot {
	return device->image_storage_slot(image);
}

auto gse::gpu::dx12_device_backend::image_format_of(const gpu::handle<gpu::image> image) const -> gpu::image_format {
	return device->image_format_of(image);
}

auto gse::gpu::dx12_device_backend::image_extent(const gpu::handle<gpu::image> image) const -> vec3u {
	return device->image_extent(image);
}

auto gse::gpu::dx12_device_backend::image_view_of(const gpu::handle<gpu::image> image) const -> gpu::handle<gpu::image_view> {
