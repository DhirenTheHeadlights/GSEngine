...
export namespace gse::gpu {
	[[nodiscard]] auto begin_transient(gpu::device& dev, queue_id id, std::string_view tag = "transient.untagged")
		-> begin_transient_awaiter;

	[[nodiscard]] auto submit(gpu::device& dev, vulkan::transient_command_buffer&& cmd, queue_id id) -> submission;

	auto dispatch(gpu::device& dev, async::task<> task) -> void;
}

auto gse::gpu::begin_transient(gpu::device& dev, const queue_id id, const std::string_view tag)
	-> begin_transient_awaiter {
