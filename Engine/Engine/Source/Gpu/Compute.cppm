...
export namespace gse::gpu {
	...
	public:
		...
		auto operator=(
			compute_queue&&
		) noexcept -> compute_queue& = default;
...
namespace gse {
	auto barrier_scope_to_gpu(
		gpu::barrier_scope scope
	) -> gpu::memory_barrier;
}

auto gse::barrier_scope_to_gpu(const gpu::barrier_scope scope) -> gpu::memory_barrier {
...
auto gse::gpu::compute_queue::submit() -> void {
	...
	m_ctx.reset_fence();
	m_ctx.submit(m_timeline_value, pipeline_stage_flag::all_commands);
	++m_frame_count;
}

auto gse::gpu::compute_queue::semaphore_state() const -> compute_semaphore_state {
	return compute_semaphore_state(&m_ctx.raii_timeline(), m_timeline_value, pipeline_stage_flag::compute_shader);
}
...
auto gse::gpu::compute_queue::barrier(const barrier_scope scope) const -> void {
	...
	const dependency_info dep{ .memory_barriers = std::span(&b, 1) };
	const auto cmd = m_ctx.command_buffer_handle();
...
auto gse::gpu::compute_queue::barriers(const std::span<const barrier_scope> scopes) const -> void {
	...
	const dependency_info dep{ .memory_barriers = barriers };
	const auto cmd = m_ctx.command_buffer_handle();
	vulkan::commands(cmd).pipeline_barrier(dep);
}
