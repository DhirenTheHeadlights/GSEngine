...
export namespace gse::gpu {
	...
	struct typed_push_constants {
		...
		auto replay(handle<command_buffer> cmd, handle<pipeline_layout> layout) const -> void;
	};
}

template <typename T>
auto gse::gpu::typed_push_constants<T>::replay(
	const handle<command_buffer> cmd,
	const handle<pipeline_layout> layout
) const -> void {
