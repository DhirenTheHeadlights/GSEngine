export module gse.gpu:vulkan_video_encoder;
...
namespace gse::gpu {
	...
	constexpr vk::ImageSubresourceRange color_subresource_range{ .aspectMask = vk::ImageAspectFlagBits::eColor,
																 .baseMipLevel = 0,
																 .levelCount = 1,
																 .baseArrayLayer = 0,
																 .layerCount = 1 };
...
auto gse::gpu::video_encoder::probe(device& dev) -> encode_capabilities {
	if (!dev.vulkan_queue().has_video_encode()) {
		return {};
	}
...
auto gse::gpu::video_encoder::create(device& dev, const vec2u extent, const encode_capabilities& probe_caps)
	-> video_encoder {
	video_encoder enc;
	enc.m_device = &dev;
	...
	vk::VideoProfileListInfoKHR profile_list{ .profileCount = 1, .pProfiles = &chain.profile };
	...
	for (const auto& req : enc.m_session.getMemoryRequirements()) {
		...
		auto mem =
			(*vk_dev).allocateMemory({ .allocationSize = req.memoryRequirements.size, .memoryTypeIndex = mem_type });

		enc.m_session.bindMemory(
			vk::BindVideoSessionMemoryInfoKHR{ .memoryBindIndex = req.memoryBindIndex,
											   .memory = mem,
											   .memoryOffset = 0,
	...
	if (probe_caps.codec == video_codec::h265) {
		...
		static vk::VideoEncodeH265SessionParametersAddInfoKHR h265_add{ .stdSPSCount = 1,
																		.pStdSPSs = sps,
																		.stdPPSCount = 1,
																		.pStdPPSs = pps };

		auto info = vk::VideoEncodeH265SessionParametersCreateInfoKHR{ .maxStdSPSCount = 1,
																	   .maxStdPPSCount = 1,
		...
		static vk::video::AV1SequenceHeader seq_header{ .seq_profile = vk::video::AV1Profile::eMain,
														.max_frame_width_minus_1 =
															static_cast<std::uint16_t>(extent.x() - 1),
														.max_frame_height_minus_1 =
															static_cast<std::uint16_t>(extent.y() - 1) };

		auto info = vk::VideoEncodeAV1SessionParametersCreateInfoKHR{ .pStdSequenceHeader = seq_header };
	...
	vk::QueryPoolVideoEncodeFeedbackCreateInfoKHR feedback_info{ .pNext = &chain.profile,
																 .encodeFeedbackFlags = feedback_flags };

	for (auto& slot : enc.m_slots) {
		slot.pool = vk_dev.createCommandPool(
			{ .flags = vk::CommandPoolCreateFlagBits::eResetCommandBuffer, .queueFamilyIndex = encode_family }
		);

		auto bufs = vk_dev.allocateCommandBuffers(
			{ .commandPool = *slot.pool, .level = vk::CommandBufferLevel::ePrimary, .commandBufferCount = 1 }
		);
		...
		slot.query_pool = vk_dev.createQueryPool(
			{ .pNext = &feedback_info, .queryType = vk::QueryType::eVideoEncodeFeedbackKHR, .queryCount = 1 }
		);

		slot.bitstream = dev.allocator().create_buffer(
			gpu::buffer_create_info{ .size = bitstream_buffer_size,
...
auto gse::gpu::video_encoder::encode_frame(const std::uint32_t frame_slot, const image& y_plane, const image& uv_plane)
	-> void {
	auto& slot = m_slots[frame_slot];
	const auto& vk_dev = m_device->vulkan_device().raii_device();

	if (slot.submitted) {
		if (vk_dev.waitForFences(*slot.fence, vk::True, std::numeric_limits<std::uint64_t>::max()) !=
			vk::Result::eSuccess) {
			return;
		}
	...
	constexpr vk::ImageSubresourceLayers y_subresource{ .aspectMask = vk::ImageAspectFlagBits::ePlane0,
														.mipLevel = 0,
														.baseArrayLayer = 0,
														.layerCount = 1 };

	constexpr vk::ImageSubresourceLayers uv_subresource{ .aspectMask = vk::ImageAspectFlagBits::ePlane1,
														 .mipLevel = 0,
														 .baseArrayLayer = 0,
														 .layerCount = 1 };

	constexpr vk::ImageSubresourceLayers src_subresource{ .aspectMask = vk::ImageAspectFlagBits::eColor,
														  .mipLevel = 0,
														  .baseArrayLayer = 0,
														  .layerCount = 1 };

	const std::array pre_barriers = { vk::ImageMemoryBarrier2{ .srcStageMask = vk::PipelineStageFlagBits2::eNone,
									  ...
									  vk::ImageMemoryBarrier2{ .srcStageMask = vk::PipelineStageFlagBits2::eNone,
															   .srcAccessMask = vk::AccessFlagBits2::eNone,
															   .dstStageMask = vk::PipelineStageFlagBits2::eTransfer,
	...
	slot.cmd.copyImage(
		std::bit_cast<vk::Image>(y_plane.handle()),
		vk::ImageLayout::eTransferSrcOptimal,
		slot.nv12_image,
		vk::ImageLayout::eTransferDstOptimal,
		vk::ImageCopy{ .srcSubresource = src_subresource,
					   .dstSubresource = y_subresource,
					   .extent = { m_extent.x(), m_extent.y(), 1 } }
	);

	slot.cmd.copyImage(
		std::bit_cast<vk::Image>(uv_plane.handle()),
		vk::ImageLayout::eTransferSrcOptimal,
		slot.nv12_image,
		vk::ImageLayout::eTransferDstOptimal,
		vk::ImageCopy{ .srcSubresource = src_subresource,
	...
	vk::VideoPictureResourceInfoKHR src_picture{ .codedExtent = { m_extent.x(), m_extent.y() },
												 .baseArrayLayer = 0,
												 .imageViewBinding = *slot.nv12_view };

	vk::VideoPictureResourceInfoKHR dpb_picture{ .codedExtent = { m_extent.x(), m_extent.y() },
												 .baseArrayLayer = 0,
												 .imageViewBinding = *target_dpb.view };

	vk::VideoPictureResourceInfoKHR ref_picture{ .codedExtent = { m_extent.x(), m_extent.y() },
												 .baseArrayLayer = 0,
	...
	const vk::VideoEncodeAV1DpbSlotInfoKHR setup_av1_dpb{ .pStdReferenceInfo = setup_av1_std };
	const vk::video::EncodeAV1ReferenceInfo ref_av1_std{ .frame_type = ref_dpb.av1_frame_type,
														 .OrderHint = ref_dpb.av1_order_hint };
	const vk::VideoEncodeAV1DpbSlotInfoKHR ref_av1_dpb{ .pStdReferenceInfo = ref_av1_std };
	...
	const vk::VideoEncodeH265DpbSlotInfoKHR setup_h265_dpb{ .pStdReferenceInfo = setup_h265_std };
	const vk::video::EncodeH265ReferenceInfo ref_h265_std{ .pic_type = ref_dpb.h265_pic_type,
														   .PicOrderCntVal = ref_dpb.h265_poc };
	const vk::VideoEncodeH265DpbSlotInfoKHR ref_h265_dpb{ .pStdReferenceInfo = ref_h265_std };
	...
	vk::VideoReferenceSlotInfoKHR setup_ref{ .pNext = setup_dpb_pnext,
	...
	if (!is_keyframe) {
		...
		if (target_dpb.active) {
			begin_ref_slots.push_back(
				{ .slotIndex = static_cast<std::int32_t>(dpb_index), .pPictureResource = nullptr }
			);
		}
	}
	...
	const vk::MemoryBarrier2 encode_sync{ .srcStageMask = vk::PipelineStageFlagBits2::eVideoEncodeKHR,
	...
	if (m_codec == video_codec::h265) {
		...
		const vk::video::H265ShortTermRefPicSet strps{ .used_by_curr_pic_s0_flag =
														   use_reference ? std::uint16_t{ 0x1 } : std::uint16_t{ 0 },
													   .num_negative_pics =
														   use_reference ? std::uint8_t{ 1 } : std::uint8_t{ 0 } };
		...
		const vk::VideoEncodeH265NaluSliceSegmentInfoKHR nalu{ .pStdSliceSegmentHeader = slice_header };

		const vk::VideoEncodeH265PictureInfoKHR h265_pic{ .naluSliceSegmentEntryCount = 1,
														  .pNaluSliceSegmentEntries = &nalu,
														  .pStdPictureInfo = std_pic_info };

		const vk::VideoReferenceSlotInfoKHR encode_ref_slot{ .pNext = &ref_h265_dpb,
															 .slotIndex = static_cast<std::int32_t>(ref_index),
															 .pPictureResource = &ref_picture };
		...
		vk::VideoEncodeAV1PictureInfoKHR av1_pic{ .predictionMode = use_reference
		...
		const vk::VideoReferenceSlotInfoKHR encode_ref_slot{ .pNext = &ref_av1_dpb,
															 .slotIndex = static_cast<std::int32_t>(ref_index),
															 .pPictureResource = &ref_picture };
...
auto gse::gpu::build_profile(profile_chain& chain, const video_codec codec) -> void {
	...
	chain.usage = { .videoUsageHints = vk::VideoEncodeUsageFlagBitsKHR::eRecording,
