...
auto gse::shader_compile::to_vertex_format(slang::TypeReflection* type) -> gpu::vertex_format {
	...
	auto elem_and_count = [](slang::TypeReflection* t) -> std::pair<slang::TypeReflection*, int> {
		switch (t->getKind()) {
			case kind::Vector:
				return { t->getElementType(), static_cast<int>(t->getElementCount()) };
			case kind::Scalar:
				return { t, 1 };
			default:
				return { t, 0 };
	...
	switch (elem->getScalarType()) {
		case slang::TypeReflection::Float32:
			switch (count) {
				case 1:
					return gpu::vertex_format::r32_sfloat;
				case 2:
					return gpu::vertex_format::r32g32_sfloat;
				case 3:
					return gpu::vertex_format::r32g32b32_sfloat;
				case 4:
					return gpu::vertex_format::r32g32b32a32_sfloat;
				default:
					return gpu::vertex_format::r32_sfloat;
			}
		case slang::TypeReflection::Int32:
			switch (count) {
				case 1:
					return gpu::vertex_format::r32_sint;
				case 2:
					return gpu::vertex_format::r32g32_sint;
				case 3:
					return gpu::vertex_format::r32g32b32_sint;
				case 4:
					return gpu::vertex_format::r32g32b32a32_sint;
				default:
					return gpu::vertex_format::r32_sint;
			}
		case slang::TypeReflection::UInt32:
			switch (count) {
				case 1:
					return gpu::vertex_format::r32_uint;
				case 2:
					return gpu::vertex_format::r32g32_uint;
				case 3:
					return gpu::vertex_format::r32g32b32_uint;
				case 4:
					return gpu::vertex_format::r32g32b32a32_uint;
				default:
					return gpu::vertex_format::r32_uint;
			}
		default:
		...
		for (int range_index = 0; range_index < binding_range_count; ++range_index) {
			...
			auto try_structured_buffer_block = [&](
				...
				if (elem_stride == 0 && !members.empty()) {
					...
					for (const auto& m : std::views::values(members)) {
...
auto gse::asset_compiler<gse::shader>::compile_one(const std::filesystem::path& source, const std::filesystem::path& destination) -> bool {
	...
	for (auto& [type, bindings] : std::views::values(sets)) {
		for (auto& b : bindings) {
