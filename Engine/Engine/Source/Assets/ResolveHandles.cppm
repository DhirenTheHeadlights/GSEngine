...
template <typename T>
auto gse::asset::resolve(resource::handle<T>& h, const shared_view<registry> assets) -> void {
	if (h.id().exists()) {
...
template <typename T>
auto gse::asset::resolve_handles(T& c, const shared_view<registry> assets) -> void {
