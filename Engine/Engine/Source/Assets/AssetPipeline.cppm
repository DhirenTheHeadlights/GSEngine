...
template <typename Resource>
	requires gse::has_asset_compiler<Resource>
auto gse::asset_pipeline::register_compiler_only() -> void {
	std::lock_guard lock(m_mutex);
		...
		for (const auto& entry : std::filesystem::recursive_directory_iterator(source_path)) {
			...
			if (!needs_compile) {
				for (const auto& dep : compiler.dependencies_fn(file_path)) {
					if (std::filesystem::exists(dep) && std::filesystem::last_write_time(dep) > std::filesystem::last_write_time(dest)) {
						needs_compile = true;
						break;
					}
				}
