...
export namespace gse::vulkan {
	...
	private:
		sync(
			device& dev,
			std::vector<gpu::semaphore_handle>&& image_available_semaphores,
			std::vector<gpu::semaphore_handle>&& render_finished_semaphores,
			std::array<std::vector<vk::raii::Fence>, gpu::queue_type_count>&& in_flight_fences
		);

		device* m_device = nullptr;
		std::vector<gpu::semaphore_handle> m_image_available;
		std::vector<gpu::semaphore_handle> m_render_finished;
		std::array<std::vector<vk::raii::Fence>, gpu::queue_type_count> m_in_flight;
	};
}

gse::vulkan::sync::sync(device& dev, std::vector<gpu::semaphore_handle>&& image_available_semaphores, std::vector<gpu::semaphore_handle>&& render_finished_semaphores, std::array<std::vector<vk::raii::Fence>, gpu::queue_type_count>&& in_flight_fences)
...
auto gse::vulkan::sync::create(device& dev, const std::uint32_t image_count, const std::uint32_t frames_in_flight) -> sync {
	std::vector<gpu::semaphore_handle> image_available;
	std::vector<gpu::semaphore_handle> render_finished;
	std::array<std::vector<vk::raii::Fence>, gpu::queue_type_count> in_flight_fences;
...
auto gse::vulkan::sync::image_available(const std::uint32_t frame_index) const -> gpu::semaphore_handle {
	return m_image_available[frame_index];
}

auto gse::vulkan::sync::render_finished(const std::uint32_t image_index) const -> gpu::semaphore_handle {
	return m_render_finished[image_index];
}

auto gse::vulkan::sync::in_flight_fence(const gpu::queue_type queue, const std::uint32_t frame_index) const -> gpu::fence_handle {
	return std::bit_cast<gpu::fence_handle>(*m_in_flight[static_cast<std::size_t>(queue)][frame_index]);
}
