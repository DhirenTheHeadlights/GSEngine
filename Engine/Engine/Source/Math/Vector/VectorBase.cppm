export module gse.math:vector_base;
...
export namespace gse::vec {
	...
	public:
		...
		constexpr base(const base<T, M>& other) requires (M <= N);

		template <internal::is_vec_element T2>
		constexpr base(const base<T2, N>& other);
		...
		constexpr auto operator+(
			...
			const base<T2, M>& rhs
		) requires (N == M && are_addable<T, T2>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator-(
			this const Self& self,
			const base<T2, M>& rhs
		) requires (N == M && are_subtractable<T, T2>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator*(
			this const Self& self,
			const base<T2, M>& rhs
		) requires (N == M && are_multipliable<T, T2>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator/(
			this const Self& self,
			const base<T2, M>& rhs
		) requires (N == M && are_divisible<T, T2>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator+=(
			this Self& self,
			const base<T2, M>& rhs
		) -> Self& requires (N == M && are_addable<T, T2> && std::same_as<add_exposed_t<T, T2>, T>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator-=(
			this Self& self,
			const base<T2, M>& rhs
		) -> Self& requires (N == M && are_subtractable<T, T2> && std::same_as<sub_exposed_t<T, T2>, T>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator*=(
			this Self& self,
			const base<T2, M>& rhs
		) -> Self& requires (N == M && are_multipliable<T, T2> && std::same_as<mul_exposed_t<T, T2>, T>);

		template <typename Self, internal::is_vec_element T2, std::size_t M>
		constexpr auto operator/=(
			this Self& self,
			const base<T2, M>& rhs
		) -> Self& requires (N == M && are_divisible<T, T2> && std::same_as<div_exposed_t<T, T2>, T>);

		template <typename Self, internal::is_vec_element S>
		constexpr auto operator*(
			this const Self& self,
			const S& rhs
		) requires are_multipliable<T, S>;

		template <typename Self, internal::is_vec_element S>
		constexpr auto operator/(
			this const Self& self,
			const S& rhs
		) requires are_divisible<T, S>;

		template <typename Self, internal::is_vec_element S>
		constexpr auto operator*=(
			this Self& self,
			const S& rhs
		) -> Self& requires (are_multipliable<T, S> && std::same_as<mul_exposed_t<T, S>, T>);

		template <typename Self, internal::is_vec_element S>
		constexpr auto operator/=(
			this Self& self,
			const S& rhs
		) -> Self& requires (are_divisible<T, S> && std::same_as<div_exposed_t<T, S>, T>);
...
constexpr gse::vec::base<T, N>::base(const T& value) {
	this->data.fill(value);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr gse::vec::base<T, N>::base(const T* values) {
	std::copy_n(values, N, this->data.begin());
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr gse::vec::base<T, N>::base(const storage<T, N>& data) : storage<T, N>(data) {}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr gse::vec::base<T, N>::base(const std::array<T, N>& data) : storage<T, N>{ data } {}

template <gse::internal::is_vec_element T, std::size_t N>
template <typename V> requires (sizeof(V) < sizeof(T))
	constexpr gse::vec::base<T, N>::base(const storage<V, N>& data) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename V> requires (sizeof(V) < sizeof(T))
	constexpr gse::vec::base<T, N>::base(const std::array<V, N>& data) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename... Args>
constexpr gse::vec::base<T, N>::base(Args&&... args)
...
template <gse::internal::is_vec_element T, std::size_t N>
template <std::size_t M>
constexpr gse::vec::base<T, N>::base(const base<T, M>& other) requires (M <= N) {
...
constexpr gse::vec::base<T, N>::base(const base<T2, N>& other) {
...
template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::at(this auto& self, std::size_t index) {
	return self.data.at(index);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::operator[](this auto& self, std::size_t index) {
	return self.data[index];
}

template <gse::internal::is_vec_element T, std::size_t N>
template <typename E> requires std::is_enum_v<E>
constexpr decltype(auto) gse::vec::base<T, N>::operator[](this auto& self, E index) {
	return self.at(static_cast<std::size_t>(index));
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::x(this auto&& self) requires(N > 0) {
	return self.at(0);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::y(this auto&& self) requires(N > 1) {
	return self.at(1);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::z(this auto&& self) requires(N > 2) {
	return self.at(2);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::w(this auto&& self) requires(N > 3) {
	return self.at(3);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::r(this auto&& self) requires (N > 0) {
	return self.at(0);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::g(this auto&& self) requires (N > 1) {
	return self.at(1);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::b(this auto&& self) requires (N > 2) {
	return self.at(2);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr decltype(auto) gse::vec::base<T, N>::a(this auto&& self) requires (N > 3) {
	return self.at(3);
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr auto gse::vec::base<T, N>::begin(this auto&& self) {
	return self.data.begin();
}

template <gse::internal::is_vec_element T, std::size_t N>
constexpr auto gse::vec::base<T, N>::end(this auto&& self) {
	return self.data.end();
}

template <gse::internal::is_vec_element T, std::size_t N>
auto gse::vec::base<T, N>::as_storage_span(this auto&& self) {
...
constexpr auto gse::vec::base<T, N>::operator+(this const Self& self, const base<T2, M>& rhs) requires (N == M && are_addable<T, T2>) {
	using R = add_exposed_t<T, T2>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator-(this const Self& self, const base<T2, M>& rhs) requires (N == M && are_subtractable<T, T2>) {
	using R = sub_exposed_t<T, T2>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator*(this const Self& self, const base<T2, M>& rhs) requires (N == M && are_multipliable<T, T2>) {
	using R = mul_exposed_t<T, T2>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator/(this const Self& self, const base<T2, M>& rhs) requires (N == M && are_divisible<T, T2>) {
	using R = div_exposed_t<T, T2>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator+=(this Self& self, const base<T2, M>& rhs) -> Self& requires (N == M && are_addable<T, T2> && std::same_as<add_exposed_t<T, T2>, T>) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator-=(this Self& self, const base<T2, M>& rhs) -> Self& requires (N == M && are_subtractable<T, T2> && std::same_as<sub_exposed_t<T, T2>, T>) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator*=(this Self& self, const base<T2, M>& rhs) -> Self& requires (N == M && are_multipliable<T, T2> && std::same_as<mul_exposed_t<T, T2>, T>) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element T2, std::size_t M>
constexpr auto gse::vec::base<T, N>::operator/=(this Self& self, const base<T2, M>& rhs) -> Self& requires (N == M && are_divisible<T, T2> && std::same_as<div_exposed_t<T, T2>, T>) {
...
constexpr auto gse::vec::base<T, N>::operator*(this const Self& self, const S& rhs) requires are_multipliable<T, S> {
	using R = mul_exposed_t<T, S>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element S>
constexpr auto gse::vec::base<T, N>::operator/(this const Self& self, const S& rhs) requires are_divisible<T, S> {
	using R = div_exposed_t<T, S>;
	std::conditional_t<std::same_as<R, T>, Self, base<R, N>> out{};
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element S>
constexpr auto gse::vec::base<T, N>::operator*=(this Self& self, const S& rhs) -> Self& requires (are_multipliable<T, S> && std::same_as<mul_exposed_t<T, S>, T>) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self, gse::internal::is_vec_element S>
constexpr auto gse::vec::base<T, N>::operator/=(this Self& self, const S& rhs) -> Self& requires (are_divisible<T, S> && std::same_as<div_exposed_t<T, S>, T>) {
...
template <gse::internal::is_vec_element T, std::size_t N>
template <typename Self>
constexpr auto gse::vec::base<T, N>::operator+(this const Self& self) -> Self {
	return self;
}
...
constexpr auto gse::vec::operator*(const S& lhs, const V& rhs) {
	return rhs * lhs;
}
