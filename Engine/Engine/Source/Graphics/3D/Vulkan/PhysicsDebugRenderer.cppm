...
export namespace gse::renderer {
	class physics_debug final : public base_renderer {
	public:
	...
	private:
		...
		static auto build_contact_debug_for_collider(
			const physics::collision_component& coll,
			const physics::motion_component& mc, std::vector<debug_vertex>& out_vertices
		) -> void;
	};
}

gse::renderer::physics_debug::physics_debug(context& context) : base_renderer(context) {
}
...
auto gse::renderer::physics_debug::update(const std::span<const std::reference_wrapper<registry>> registries) -> void {
	...
	for (const auto& reg_ref : registries) {
		for (auto& reg = reg_ref.get(); const auto& coll : reg.linked_objects_read<physics::collision_component>()) {
			if (!coll.resolve_collisions) {
				continue;
			}
			build_obb_lines_for_collider(coll, vertices);
			build_contact_debug_for_collider(coll, reg.linked_object_read<physics::motion_component>(coll.owner_id()), vertices);
		}
	}
...
auto gse::renderer::physics_debug::render(std::span<const std::reference_wrapper<registry>> registries) -> void {
	if (!m_enabled) {
		return;
	}
