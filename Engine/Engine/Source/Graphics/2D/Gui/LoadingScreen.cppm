...
gse::gui::loading_screen::loading_screen(const loading::state& state)
	: m_state(&state) {}
...
auto gse::gui::loading_screen::build(builder& ui, nav& n) -> void {
	...
	const float fill_ratio = total == 0
		? 0.f
		: std::clamp(static_cast<float>(done) / static_cast<float>(total), 0.f, 1.f);
