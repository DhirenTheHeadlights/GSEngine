...
auto gse::cursor::render_to(const asset::registry::state& assets, std::vector<renderer::sprite_command>& commands, const vec2f mouse_pos) -> void {
	const resource::handle<texture> blank_texture = asset::registry::get<texture>(assets, find("blank"));
