...
auto gse::menu_bar::update(state& state, const context& ctx, const input::state& input, const vec2f viewport_size) -> void {
	...
	ctx.sprites.push_back({ .rect = bar, .color = sty.color_menu_bar, .texture = ctx.blank_texture });

	if (ctx.font.valid()) {
		ctx.texts.push_back({ .font = ctx.font, .text = "GSEngine", .position = { bar.left() + sty.padding, bar.center().y() + sty.font_size * 0.35f }, .scale = sty.font_size, .color = sty.color_text, .clip_rect = bar });
	}
...
auto gse::menu_bar::draw_gear_icon(std::vector<renderer::sprite_command>& commands, const vec2f center, const float size, const vec4f color, const resource::handle<texture> texture) -> void {
	...
	for (int i = 0; i < tooth_count; ++i) {
		...
		commands.push_back({ .rect = gui::ui_rect::from_position_size({ mid.x() - thickness * 0.5f, mid.y() + len * 0.5f }, { thickness, len }), .color = color, .texture = texture, .rotation = angle{ angle_rad } });
	}
