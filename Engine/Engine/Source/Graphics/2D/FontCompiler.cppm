...
export namespace gse {
	...
	auto bake(const std::filesystem::path& src, font::baked& out) -> bool;
}
...
auto gse::bake(const std::filesystem::path& src, font::baked& out) -> bool {
	...
	constexpr float glyph_cell_size = 128.f;
	...
	const msdfgen::Range pixel_range(8.0);
	int glyph_index = 0;
	...
	for (int c = first_char; c <= last_char; ++c, ++glyph_index) {
		...
		msdfgen::Bitmap<float, 4> mtsdf_bitmap(glyph_cell_size, glyph_cell_size);
		...
		generateMTSDF(mtsdf_bitmap, shape, pixel_range, { scale, scale }, { tx, ty });

		for (int y = 0; y < mtsdf_bitmap.height(); ++y) {
			for (int x = 0; x < mtsdf_bitmap.width(); ++x) {
				...
				atlas_data[idx + 0] = static_cast<std::byte>(std::clamp(mtsdf_bitmap(x, y)[0], 0.f, 1.f) * 255.f);
				atlas_data[idx + 1] = static_cast<std::byte>(std::clamp(mtsdf_bitmap(x, y)[1], 0.f, 1.f) * 255.f);
				atlas_data[idx + 2] = static_cast<std::byte>(std::clamp(mtsdf_bitmap(x, y)[2], 0.f, 1.f) * 255.f);
				atlas_data[idx + 3] = static_cast<std::byte>(std::clamp(mtsdf_bitmap(x, y)[3], 0.f, 1.f) * 255.f);
			}
		}
		...
		glyphs[static_cast<char>(c)] = glyph(
			glyph::info{
