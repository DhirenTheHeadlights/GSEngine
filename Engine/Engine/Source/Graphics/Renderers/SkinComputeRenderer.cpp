...
auto gse::renderer::skin_compute::system::run(
	...
	assert(
		static_cast<bool>(gc.skeleton_buffer),
		"skin_compute::initialize: gc.skeleton_buffer is null - geometry_collector::initialize did not run before "
		"skin_compute::initialize"
	);
	...
	for (std::size_t i = 0; i < per_frame_resource<gpu::descriptor_region>::frames_in_flight; ++i) {
		...
		gpu::descriptor_writer(gpu::context::device_handle(*gpu_s.device), d.descriptors[i])
			.buffer<skeleton_data>(
				gc.skeleton_buffer,
				0,
				geometry_collector::system::data::max_joints * sizeof(geometry_collector::joint_data)
			)
...
auto gse::renderer::skin_compute::system::frame(
	frame_context& ctx,
