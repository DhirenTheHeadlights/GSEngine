...
namespace gse {
	...
	template <auto MemberFn, typename S, typename State>
	consteval auto compute_state_dep_count(
	) -> std::size_t;

	template <auto MemberFn, typename S, typename State>
	consteval auto compute_state_dep_ids(
	) -> std::array<id, compute_state_dep_count<MemberFn, S, State>()>;
...
consteval auto gse::compute_state_dep_count() -> std::size_t {
	...
	for (auto p : std::meta::parameters_of(MemberFn)) {
		...
		const bool is_dep = std::meta::extract<bool>(
			std::meta::substitute(^^is_state_dep_v, { t, ^^S, ^^State })
		);
		if (is_dep) {
			++count;
		}
...
template <auto MemberFn, typename S, typename State>
consteval auto gse::compute_state_dep_ids() -> std::array<id, compute_state_dep_count<MemberFn, S, State>()> {
	std::array<id, compute_state_dep_count<MemberFn, S, State>()> result{};
	std::size_t i = 0;
	for (auto p : std::meta::parameters_of(MemberFn)) {
		auto t = std::meta::dealias(std::meta::type_of(p));
		const bool is_dep = std::meta::extract<bool>(
...
auto gse::extract_update_state_deps() -> std::vector<id> {
	if constexpr (!names_update<S>) {
		...
		constexpr auto deps = compute_state_dep_ids<^^S::update, S, State>();
...
auto gse::extract_frame_state_deps() -> std::vector<id> {
	if constexpr (!names_frame<S>) {
		...
		constexpr auto deps = compute_state_dep_ids<^^S::frame, S, State>();
		return std::vector<id>(deps.begin(), deps.end());
	}
}
...
auto gse::invoke_shutdown_for(shutdown_context& phase, void* data_ptr) -> void {
	auto& d = *static_cast<system_node_data<S, State>*>(data_ptr);
	if constexpr (shutdown_takes_resources_state<S, State>) {
		S::shutdown(phase, d.resources.value, d.state);
		return;
	}
	else if constexpr (shutdown_takes_state<S, State>) {
