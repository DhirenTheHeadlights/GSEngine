...
namespace gse {
	template <typename S>
	using state_of_t = typename S::state;
	...
	template <typename T>
	constexpr id state_dep_id_v = id_of<std::remove_cvref_t<T>>();
...
template <typename Arg, typename S>
auto gse::resolve_initialize_arg(init_context& phase, resource_storage<S>& resources, update_data_storage<S>& update_data, frame_data_storage<S>& frame_data, state_of_t<S>& state) -> decltype(auto) {
	using U = std::remove_cvref_t<Arg>;
...
auto gse::direct_state_ref(const task_context& ctx) -> const T& {
	const auto* p = ctx.live_state
		? ctx.states.state_ptr(id_of<T>())
		: ctx.states.state_snapshot_ptr(id_of<T>());
...
template <typename Arg, typename S>
auto gse::resolve_update_arg(update_context& ctx, resource_storage<S>& resources, update_data_storage<S>& update_data, frame_data_storage<S>& frame_data, state_of_t<S>& state) -> decltype(auto) {
	using U = std::remove_cvref_t<Arg>;
	if constexpr (std::is_same_v<U, update_context>) {
		return (ctx);
	}
...
template <typename Arg, typename S>
auto gse::resolve_frame_arg(frame_context& ctx, resource_storage<S>& resources, frame_data_storage<S>& frame_data, const state_of_t<S>& state) -> decltype(auto) {
	using U = std::remove_cvref_t<Arg>;
	if constexpr (std::is_same_v<U, frame_context>) {
		...
		static_assert(
			...
			"frame() parameter S must be trivially-copyable so a per-frame "
			"snapshot exists. Make S POD-trivial, or remove the parameter — "
			"frame() runs concurrently with the next frame's update(), so a "
			"non-snapshot read is a data race."
		);
		return direct_state_ref<U>(ctx);
	}
}
...
auto gse::make_system_node(Args&&... args) -> system_node {
	...
	node.state_id = id_of<state_of_t<S>>();
