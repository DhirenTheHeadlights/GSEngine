...
auto gse::broad_phase_collision::check_collision(physics::collision_component& dynamic_object_collision_component, physics::motion_component* dynamic_object_motion_component, const physics::collision_component& other_collision_component, const time dt) -> void {
	...
	if (check_future_collision(box1, dynamic_object_motion_component, box2, dt)) {
		if (dynamic_object_collision_component.resolve_collisions) {
			narrow_phase_collision::resolve_collision(dynamic_object_motion_component, dynamic_object_collision_component, other_collision_component);
		}
	}
}
...
auto gse::broad_phase_collision::update(const std::span<physics::collision_component> collision_components, const std::span<physics::motion_component> motion_components, const time dt) -> void {
	...
	for (auto& collision_component : collision_components) {
		...
		const auto it = std::ranges::find_if(motion_components, [&](const physics::motion_component& motion_component) {
				return motion_component.parent_id == collision_component.parent_id;
			});
		...
		for (auto& other_collision_component : collision_components) {
			if (collision_component.parent_id == other_collision_component.parent_id) {
				continue;
			}

			check_collision(collision_component, motion, other_collision_component, dt);
		}
