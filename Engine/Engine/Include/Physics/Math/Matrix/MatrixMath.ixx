...
constexpr auto gse::look_at(const vec3<length_t<T>>& position, const vec3<length_t<T>>& target, const unitless::vec3_t<T>& up) -> mat4_t<T> {
	...
	return mat4_t<T>{
		...
		unitless::vec4_t<T>{x_axis.z, y_axis.z, -z_axis.z, 0},
		unitless::vec4_t<T>{-dot(x_axis, position_in_default_units), -dot(y_axis, position_in_default_units), dot(z_axis, position_in_default_units), 1}
	};
}
