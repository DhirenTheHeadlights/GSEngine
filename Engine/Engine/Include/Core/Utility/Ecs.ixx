...
export namespace gse {
	...
	public:
		...
		auto activate(const owner_id_t& owner_id) -> bool override {
		...
		auto try_get(const owner_id_t& owner_id) -> component_type* {
		...
		auto try_get_by_link_id(const link_id_t& link_id) -> component_type* {
	...
	private:
		id_mapped_collection<component_type, owner_id_t> m_active_components;
		id_mapped_collection<component_type, owner_id_t> m_queued_components;
	...
	public:
		...
		auto hooks_as_base() -> std::vector<hook<entity>*> override {
		...
		auto try_get(const owner_id_t& owner_id) -> T* {
			if (auto* hook_ptr = m_active_hooks.try_get(owner_id)) {
				return hook_ptr->get();
			}
		...
		auto try_get_by_link_id(const link_id_t& link_id) -> T* {
			if (const auto it = m_link_to_owner_map.find(link_id); it != m_link_to_owner_map.end()) {
				return try_get(it->second);
			}
			return nullptr;
	...
	private:
		id_mapped_collection<hook_ptr_type, owner_id_t> m_active_hooks;
	...
	public:
		...
		auto add_deferred_action(const id& owner_id, deferred_action action) -> void;
		...
		auto all_hooks() -> std::vector<hook<entity>*>;
	...
	private:
		id_mapped_collection<entity, id> m_active_entities;
...
public:
	...
	template <is_component T>
	auto configure_when_present(std::function<void(T&)> config_func) -> void;
...
auto gse::hook<gse::entity>::configure_when_present(Func&& config_func) -> void {
	using c = typename lambda_traits<Func>::component_type;
...
export namespace gse {
	...
	public:
		explicit scene(const std::string& name = "Unnamed Scene") : hookable(name) {}
