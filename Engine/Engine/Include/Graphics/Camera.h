...
namespace Engine {
	...
	public:
		...
		void moveRelativeToOrigin(const Vec3<>& direction, const float distance, const float deltaTime) {
		...
		glm::mat4 getViewMatrix() const { return lookAt(position.as<Meters>(), (position + front).as<Meters>(), up.as<Meters>()); }
