...
export namespace gse::renderer {
	...
	public:
		explicit geometry(const std::unique_ptr<context>& context);
...
gse::renderer::geometry::geometry(const std::unique_ptr<context>& context) : base_renderer(context) {}
...
auto gse::renderer::geometry::render(const std::span<render_component> components) -> void {
	auto& config = m_context->config();
					...
					for (const auto& entry : model_handle.render_queue_entries()) {
						...
						if (entry.mesh->material.exists()) {
							geometry_shader->push(
								...
								m_pipeline_layout,
								"diffuse_sampler",
								m_context->resource<texture>(entry.mesh->material->diffuse_texture).descriptor_info()
							);
