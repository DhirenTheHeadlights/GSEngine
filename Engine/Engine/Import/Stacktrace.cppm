...
namespace gse {
	...
	LONG WINAPI vectored_handler(EXCEPTION_POINTERS* info) noexcept {
		...
		if (code == cpp_exception_code
			|| code == breakpoint_code
			|| code == dbg_print_exception_code
			|| code == dbg_print_exception_wide_c
			|| code == ms_thread_name_code) {
